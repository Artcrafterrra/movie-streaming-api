...
class PaymentService(PaymentServiceInterface):
    ...
    async def _load_order(self, session: Session, order_id: int) -> OrderModel:
        """Fetch order with items or raise."""
        order = await self.repo.get_order_with_items(session, order_id)
        if not order:
    ...
    async def _recalculate_and_update_order_status(
        self, session: Session, order: OrderModel
    ) -> None:
        """Recompute aggregate payment state and update order status accordingly."""
        paid_total = await self.repo.sum_successful_payments(session, order.id)
    ...
    async def create_payment(
        ...
        *,
        ...
        if external_payment_id and await self.repo.external_payment_exists(
        ...
        await self.repo.save_payment(session, payment)

        for oi in order.items:
            await self.repo.save_payment_item(
                session,
                PaymentItemModel(
        ...
        if status == PaymentStatusEnum.SUCCESSFUL:
            await self._recalculate_and_update_order_status(session, order)
    ...
    async def cancel_payment(
        ...
        payment = await self.repo.get_payment_by_id(session, payment_id)
        if not payment:
        ...
        order = await self._load_order(session, payment.order_id)
        await self._recalculate_and_update_order_status(session, order)
    ...
    async def get_user_payments(
        ...
        return await self.repo.list_user_payments(session, user_id)

    async def get_order_payments(
        self, session: Session, *, order_id: int
    ) -> list[PaymentModel]:
        """Return all payments linked to the given order."""
        return await self.repo.list_order_payments(session, order_id)

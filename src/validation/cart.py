...
async def check_purchased_movies(
    cart: Cart, cart_item: CartItem, db: AsyncSession = Depends(get_db)
):
    result = await db.execute(
        select(UserModel).where(UserModel.id == cart.user_id)
    )
    user = result.scalars().first()
    for movie in user.bought_movies:
        if cart_item.movie == movie:
            raise HTTPException(
                status_code=400, detail="Movie is already purchased"
            )
...
async def check_movie_certification(
    cart: Cart, cart_item: CartItem, db: AsyncSession = Depends(get_db)
):
    restrictions = {"G": 0, "PG": 10, "PG-13": 13, "R": 17, "NC-17": 18}
    movie_query = await db.execute(
        select(Movie).where(Movie.id == cart_item.movie_id)
    )
    ...
    if user_age < restrictions[certification]:
        raise HTTPException(
            status_code=403,
            detail="User's age is not suitable for watching current movie",
        )
